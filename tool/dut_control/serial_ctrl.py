...
class SerialCtrl:
	...
	def start_catch_kernellog(self):
		def read_serial():
			while True:
				with open('dut_kernel.log', 'w') as f:
					try:
						res = self.ser.read_all().decode('utr-8')
					except Exception:
						res = ''
					if res:
		...
		self.t = threading.Thread(target=read_serial)
		self.t.daemon = True
		self.t.start()

	def stop_catch_kernellog(self):
		self.t.join()
